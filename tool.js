...
export default {
     ...
     * 查询url参数
     * */
    hashQueryString: function(key) {
        var reg = new RegExp(key + '=([^&]*)(&|$)', 'i');
        var begin = location.hash.indexOf("?");
        var hashSearch = location.hash.slice(begin+1);
        var result = window.location.hash.substr(1).match(reg);
        if (result != null) {
